...
class UserAPI {
  ...
  // PUT add a gig object to User's appliedGigs property --------------------------
  async applyToGig(gigId) {
    //validate
    if(!gigId) return
    
    // fetch the json data
    const response = await fetch(`${App.apiBase}/user/apply`, {
      method: "PUT",
      headers: { "Authorization": `Bearer ${localStorage.accessToken}`, "Content-Type": 'application/json'},
    ...
    if(!response.ok){ 
      ...
      if(err) console.log(err)
      // throw error (exit this function)      
      throw new Error('Problem applying to gig!')
    }

    console.log("In USERAPI.applytogig")
